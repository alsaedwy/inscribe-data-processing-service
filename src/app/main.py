...
from app.core.logging import (
    ...
    log_application_startup, log_application_shutdown
)
...
def create_app() -> FastAPI:
    ...
    async def log_requests(request: Request, call_next):
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_request_end(logger, request.method, str(request.url.path), response.status_code, duration_ms)
    ...
    async def legacy_get_customers(
        username: str = Depends(authenticate_simple)
    ):
