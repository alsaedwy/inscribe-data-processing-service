...
class DatabaseSetup:
    ...
    def create_iam_user(self) -> bool:
        ...
        """
        if not all([self.master_host, self.master_username, self.master_password, self.database_name]):
        ...
        try:
            ...
            with connection:
                with connection.cursor() as cursor:
                    ...
                    connection.commit()
                    logger.info(f"Successfully created IAM database user '{self.iam_username}'")
                    return True
