...
def setup_datadog() -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).info(
            ...
            extra={"service": settings.datadog_service_name, "env": settings.datadog_env}
        )
        
    except Exception as e:
        logging.getLogger(__name__).error(
            f"Failed to initialize Datadog: {e}",
            extra={"error": str(e)}
        )
...
def log_request_end(logger: logging.Logger, method: str, path: str, status_code: int, duration_ms: float) -> None:
...
def log_database_operation(logger: logging.Logger, operation: str, table: str, success: bool, duration_ms: Optional[float] = None) -> None:
